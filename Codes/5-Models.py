...
# Regression function with NumPy
...
def draw_fx(fx, acc_dict, num_points=100, title=None, path_save_fig=None):
    ...
    plt.plot(x_vals, y_vals, label=f"y = {sp.simplify(fx)}", color='blue')  # Regression line
    plt.scatter(dict_x, dict_y, color='red', label='Original Data')  # Original data points
...
def train_catboost_(df: pd.DataFrame, iterations: int, features: list, target: str, cat_features: list = None,
    ...
    """
    Function to train a CatBoost model and evaluate it with various metrics based on the task type.
    :param df: DataFrame with the data
    :param features: List of feature column names
    :param target: Name of the target column
    :param cat_features: List of names of categorical variables (optional)
    :param task: Task type - "classification" or "regression"
    :param tree_idx: Tree number for plotting
    :return: The trained model, metrics, and the training set
    """
    ...
    # Display the number of trees in the model
...
def plot_feature_importance(model, features):
    ...
    if len(feature_importance) != len(features):
        ...
        features = model.feature_names_  # Get feature names directly from the model
    sorted_idx = np.argsort(feature_importance)  # Sort features
    # Bar plot
...
def run_extra_trees_model(df, target_column, T=0.2, N=400, D=10):
    ...
    # One-Hot Encoding for all categorical features except the target column
...
def run_lightgbm_model(df, T=3):
    ...
    # Convert all categorical variables to categories (LightGBM supports this natively)
...
print('-'*20 + '🚀 XGBoost Model Information ' + '-'*20)
os.makedirs('Reports/XGboost', exist_ok=True)

def train_xgboost(df, target_column, T=0.2, D=2, LR=0.1):
    # Split the data into features (X) and target column (y)
    # Automatically remove all datetime columns
    X = df.drop(columns=[target_column])
    X = X.select_dtypes(exclude=['datetime64[ns]'])
    y = df[target_column]
    # Convert the target column (y) to numbers using LabelEncoder
    target_encoder = LabelEncoder()
    y_encoded = target_encoder.fit_transform(y)
    # Convert categorical variables in features (X) to numbers
    le = LabelEncoder()
